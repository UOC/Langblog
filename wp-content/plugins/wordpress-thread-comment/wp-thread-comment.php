...
/* 
Plugin Name: LANGBLOG Wordpress Thread Comment
...
class wp_thread_comment{
	...
	function defaultoption($key=''){
		...
		if($key === 'cancel_reply'){
			...
			return  __('Reply','wp-thread-comment');
		}elseif($key === 'memory_limit'){
			return 0;
		}elseif($key === 'comment_html'){
			
			return __('<div class="comment-childs<?php echo $deep%2 ? \' chalt\' : \'\'; ?>" id="comment-[ID]"><a href="#" onclick="myplugin_cast_Read([ID],[C_ID]);return false;">[leido]</a>&nbsp;<span class="comment-metaRead">&nbsp;[date][time]&nbsp;-&nbsp;<cite>[author]</span></cite></p><p>[content]</p></div>','wp-thread-comment');
	...
	function admincommenttext($text){
		...
		$text .= '<p><a href="javascript:void(0)" onclick="popuptext(event,' . $comment->comment_post_ID . ',' . $comment->comment_ID . ',\'' . $this->encodejs(strip_tags($comment->comment_author)) . '\');">'. __('Reply','wp-thread-comment') . '</a></p>';
...
<form id="inlinereply" method="post" action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" style="display:none" onsubmit="return wptcadminajaxsend()">
	...
	<input name="cancel" id="cancel" value="<?php _e('Cancel', 'wp-thread-comment'); ?>" type="button" onclick="javascript:document.getElementById('wptc_comment_post_ID').value=0;document.getElementById('inlinereply').style.display='none'" />
...
/*	function activate(){
	...
	function addreplyidformfield(){
		...
		echo '<div id="cancel_reply" style="display:none;"><a href="javascript:void(0)" onclick="movecfm(null,0,1,null);showArrow(0);">'.__('Click to cancel reply', 'wp-thread-comment').' </a></div>';
	...
	function addchildcomment($text){
		...
		$orgcomment = $comment;
		$path2Theme = 'wp-content/themes/presenta/';
		if($this->cap['reply'] && $deep < (int) $this->options['comment_deep']){
			
			$text .= '<script type="text/javascript">flechas=flechas+"*"+'.$comment->comment_ID.';</script><p style="margin-left:-24px;">'.$this->replytext['reply_text_before'].'<a class="replyText" href="javascript:void(0)" style="text-decoration:none;" onclick="showArrow('.$comment->comment_ID.');if(arrowPressed==true){movecfm(event,' . $comment->comment_ID . ',' . ($deep+1) . ',\'' . $this->encodejs(strip_tags($comment->comment_author)) . '\');}else{movecfm(null,0,1,null);document.getElementById(\'flecha'.$comment->comment_ID.'\').src=\''.get_option('siteurl').'/'.$path2Theme.'images/noflecha.gif\';}"><img id="flecha'.$comment->comment_ID.'" src="'.get_option('siteurl').'/'.$path2Theme.'images/noflecha.gif" align="bottom">&nbsp;'. __('Reply','wp-thread-comment') . '</a>'.$this->replytext['reply_text_after'].'</p>';
		}

		if($this->options['manage_front'] === 'yes' && $this->cap['delete']){
			$text .= '<p class="thdmang">[<a href="'. wp_nonce_url($this->info['siteurl'].'/wp-admin/comment.php?action=deletecomment&amp;p=' . $comment->comment_post_ID . '&amp;c=' . $comment->comment_ID, 'delete-comment_' . $comment->comment_ID) .'" onclick="return confirm(\''. __('Do you really want to delete the comment?','wp-thread-comment') . '\');">'. __('Delete','wp-thread-comment') . '</a>] | <input type="text" size="' . strlen($comment->comment_ID) . '" class="mvccls" value="' . $comment->comment_ID . '" onkeydown="javascript:if(event.keyCode==13) {window.location=\''. wp_nonce_url($this->info['url'].'/'.$this->info['file'].'?action=movecomment&amp;p=' . $comment->comment_post_ID . '&amp;c=' . $comment->comment_ID, 'move-comment_' . $comment->comment_ID) .'&amp;pc=\'+this.value; return false;}" onfocus="javascript:this.className = \'\'; this.value = \'\';" onblur="javascript:this.className = \'mvccls\'; this.value = \''.$comment->comment_ID.'\';" /></p>';
		}
	...
	function commenttext($deep=0,$count=0){
		...
		if(function_exists('wpthreadcomment_threadedcomment')){
			...
			$p = str_replace('[C_ID]', $post->ID, $p);
			$p = str_replace('[author]', get_comment_author_link(), $p);
			$p = str_replace('[date]', get_comment_date('n/j/Y - '), $p);
			...
			$userIdFs = get_comment(get_comment_ID())->user_id;
			global $user_info;
			$user_info = wp_get_current_user();
			if(get_comment_meta(get_comment_ID(),'_AlreadyReadComm'.$user_info->ID,true)==0)
			$retCmoyas= '<img id="imgC'.get_comment_ID().'" src="'.get_bloginfo("url").'/wp-content/themes/tema_mediablog/images/noleido.gif" border="0" />';
			else $retCmoyas= '<img id="imgC'.get_comment_ID().'" src="'.get_bloginfo("url").'/wp-content/themes/tema_mediablog/images/leido.gif" border="0" />';
			$p = str_replace('[leido]', $retCmoyas, $p);
...
<?php	
	...
	function options_page(){
		...
		if(isset($_POST['updateoptions'])){
			...
			$this->resetToDefaultOptions();
			$this->message = __('Plugin confriguration has been reset back to default!','wp-thread-comment');
		}else{}
		$this->displayMessage();
?>
...
<div class="wrap">
	...
	<form method="post" action="">
		...
		<p>
			...
			<br /><br />
			<label><?php _e('Reply Text', 'wp-thread-comment'); ?>:</label>
		...
		<p>
			...
			<input type="checkbox" name="no_css" id="no_css" value="yes" checked="checked"/>
			<br />
		...
		<p>
			...
			<input type="checkbox" name="clean_option" id="clean_option" value="yes" <?php if (1==1) { ?> checked="checked"<?php } ?>/>
			<br />
