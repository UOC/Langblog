...
<div class="wrap kalturaWrapFix">
			...
			<td>
				<script type="text/javascript">
					function updateWidthHeight()
					{
						...
						<?php foreach($viewData["uiConfs"]->objects as $uiConf): ?>
	...
	<table class="widefat page fixed" cellspacing="0">
		...
		<tbody>
			<?php $alternate = true; ?>
			<?php foreach($viewData["categories"] as $category => $entries): ?>
				<?php foreach($entries as $entry): ?>
					<tr class="iedit <?php echo ($alternate) ? "alternate" : ""; $alternate = !$alternate; ?>">
						<th scope="row" class="check-column"><input type="checkbox" name="entries[]" value="<?php echo base64_encode(serialize(array($entry->id, $entry->name, $category))); ?>" /></td>
