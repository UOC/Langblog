...
use uploader_kaltura\API\KalturaPlugins as KalturaPlugins;

class UploadHandler
{
    protected $options;
// ADDED ******* UOC abertranb to upload to kaltura        
    private $file_path;
// ******** END
...
// ADDED ******* UOC abertranb to upload to kaltura        
    private function kalturaUpload($file) {
        try {
            $file->uploaded_kaltura = false;
            //require_once(dirname(__FILE__).'/../../settings.php'); 
            require_once(dirname(__FILE__).'/../../API/KalturaClient.php');
            require_once(dirname(__FILE__).'/../../lib/kaltura_html5_helpers.php');
            

            if (KalturaHTML5Helpers::videoCommentsEnabled()) {    
                $adminSecret = KalturaHTML5Helpers::getOption('kaltura_admin_secret');
                $partnerId = KalturaHTML5Helpers::getOption('kaltura_partner_id');
                $kaltura_server_url = KalturaHTML5Helpers::getOption('kaltura_server_url');
                error_log("adminSecret $adminSecret kaltura_server_url $kaltura_server_url");
                if ($adminSecret && $partnerId) {
                    ...
                    $entry = new KalturaBaseAPI\KalturaMediaEntry();

                    $entry->name = $file->name;
                    $entry->mediaType = KalturaBaseAPI\KalturaMediaType::VIDEO;

                    $result = $client->media->addFromUploadedFile($entry, $token);

                    $entryid = '';
                    if ($result && $result->rootEntryId) {
                        $entryid = $result->rootEntryId;
            ...
            /*require_once(dirname(__FILE__).'/../../API/KalturaClient.php');
            require_once(dirname(__FILE__).'/../../lib/kaltura_html5_helpers.php');
            $kconf = new /*uploader_kaltura\API\* /KalturaConfiguration($partnerId);
            ...
            $kclient = new /*uploader_kaltura\API\* /KalturaClient($kconf);
            $type = null;//KalturaSessionType::ADMIN;
            $ksession = $kclient->session->start($secret, $userId, $type, $partnerId);

            if (!isset($ksession)) {
                $msg = "Could not establish Kaltura session. Please verify that you are using valid Kaltura partner credentials.";
                $file->upload_kaltura_error = $msg;
                error_log($msg);
            } else {
                ...
                // KALTURA_SERVICE_FORMAT_PHP   php
                //if (!$loaded_API) {
                    $kconf->format = /*uploader_kaltura\API\* /KalturaClientBase::KALTURA_SERVICE_FORMAT_PHP;
                    $token = $kclient->media->upload($movie);

                    $entry = new /*uploader_kaltura\API\* /KalturaMediaEntry();

                    $entry->name = $file->name;
                    $entry->mediaType = /*uploader_kaltura\API\* /KalturaMediaType::VIDEO;
...
if (KalturaHTML5Helpers::all_in_one_is_2_5_or_newer()  && !class_exists('KalturaMediaEntry')){
